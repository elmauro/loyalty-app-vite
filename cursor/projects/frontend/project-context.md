...
> **Repositorio canónico:** este documento vive en **loyalty-cursor** (esta carpeta).  
> **Rutas** como `docs/`, `src/`, `cypress/`: bajo el repo **`loyalty-app-vite`** (colócalo junto a `loyalty-cursor` en el monorepo).
...
│   └── (project-context y cursor-rules canónicos: ../../loyalty-cursor/projects/loyalty-frontend/)
...
`docs/README.md` is the index. **Reglas y contexto para IA (Cursor):** [cursor-rules-frontend.md](cursor-rules-frontend.md) y este archivo, en el repo **loyalty-cursor** (`projects/loyalty-frontend/`). Root README in `loyalty-app-vite` points to key flows and deployment; some links may still reference old paths (e.g. `docs/DEPLOY-AWS.md` → now `docs/infrastructure/GUIA-DESPLIEGUE-AWS-FRONTEND.md`).
...
- **Conventions:** Section 11 summarizes conventions; [`cursor-rules-frontend.md`](cursor-rules-frontend.md) in **loyalty-cursor** (same directory as this file) adds short rules (e.g. API alignment, E2E/Swagger, no hardcoded config).
